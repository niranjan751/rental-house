...
function LoginForm({ onSwitch }) {
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      login({ name: email.value.split('@')[0], email: email.value, avatar: null });
  ...
  return (
    <form className="lp-form" onSubmit={submit} noValidate>
      <div className="lp-field">
        ...
        {errors.email && <span className="lp-field__error">{errors.email}</span>}
      </div>

      <div className="lp-field">
        <div className="lp-field__row">
          <label htmlFor="li-password">Password</label>
          <a href="#forgot" className="lp-field__link">Forgot password?</a>
        </div>
        <div className="lp-field__password">
          <input id="li-password" type={showPw ? 'text' : 'password'}
            autoComplete="current-password" placeholder="Enter your password"
            {...password} aria-invalid={!!errors.password} />
          <button type="button" className="lp-field__toggle"
            onClick={() => setShowPw((s) => !s)}
            aria-label={showPw ? 'Hide password' : 'Show password'}>
            {showPw ? 'Hide' : 'Show'}
          </button>
        </div>
        {errors.password && <span className="lp-field__error">{errors.password}</span>}
      </div>

      <button type="submit" className="lp-submit" disabled={busy}>
        {busy ? <><span className="lp-spinner" /> Logging in…</> : 'Log in'}
      </button>
      ...
      <p className="lp-footer">
        New to RentalHub?{' '}
        <button type="button" className="lp-auth-link" onClick={onSwitch}>
          Create an account
        </button>
      </p>
    </form>
...
function SignupForm({ onSwitch }) {
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      login({ name: name.value, email: email.value, avatar: null });
      navigate('/', { replace: true });
    } finally {
      setBusy(false);
    }
  ...
  return (
    <form className="lp-form" onSubmit={submit} noValidate>
      <div className="lp-field">
        ...
        {errors.name && <span className="lp-field__error">{errors.name}</span>}
      </div>
      ...
      <div className="lp-field">
        ...
        {errors.confirm && <span className="lp-field__error">{errors.confirm}</span>}
      </div>
...
function OAuthRow({ label = 'or continue with' }) {
  return (
    <>
      <div className="lp-divider"><span>{label}</span></div>
      <div className="lp-oauth-row">
        <button type="button" className="lp-oauth-btn">
...
export default function LoginPage() {
  ...
  return (
    <div className="lp-page">
      {/* Left panel — branding */}
      <div className="lp-panel lp-panel--left">
        ...
        <div className="lp-hero-text">
          ...
          <p>Thousands of verified rentals across hundreds of cities — all in one place.</p>
        </div>

        <ul className="lp-perks">
          <li>
            <span className="lp-perk-icon">🏠</span>
      ...
      {/* Right panel — form */}
      <div className="lp-panel lp-panel--right">
